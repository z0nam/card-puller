...
```bash
./launch_chrome_cdp.sh               # macOS/Linux: 디버깅 포트(9222) + 전용 프로필로 크롬 실행
```
```powershell
...
macOS/Linux 는 `./card …`, Windows 는 `.\card …` — **서브커맨드/인자는 동일**합니다.
...
```
한글 별칭: `card 체크`, `card 받기`, `card 증빙`. 자주 쓰면 macOS/Linux `alias card="$PWD/card"`.
...
card            # 실행 래퍼 (macOS/Linux)         card.cmd            # 실행 래퍼 (Windows)
launch_chrome_cdp.sh  # 크롬 디버깅 런처 (mac/Lin) launch_chrome_cdp.cmd  # 크롬 디버깅 런처 (Win)
```
...
Python 3.12, Playwright, pandas/pyarrow/openpyxl, lxml, img2pdf. macOS/Linux/Windows.
(Python 본체는 `pathlib`만 쓰고 경로 하드코딩이 없어 크로스플랫폼. OS별로 갈리는 건 실행 래퍼/크롬 런처뿐.)
